...
class Meteo:
    ...
    def __init__(self, inidate, enddate, region):
        ...
        self.metadata = {'region': region,  # place / reslistnamee
    ...
    def find_station(self):
        ...
        url = "/opendata/api/valores/climatologicos/inventarioestaciones/" \
            + "todasestaciones/?api_key=" + self.credentials
        ...
        for search in datosEstaciones:
            ...
            if search['longitud'][-1] == "E":
                ...
                lon_temp = float(search['longitud'][0:-1]) * -0.0001
            if (abs(self.lon - lon_temp) + abs(self.lat - lat_temp)) \
                    < min_station:
                self.station = search['indicativo']
                min_station = abs(self.lon - lon_temp) \
                    + abs(self.lat - lat_temp)
                print('Estacion %s - %s || %s, %s' % (
                    search['indicativo'],
                    search['nombre'],
                    lat_temp, lon_temp))
        return self.station
    ...
    def datosEstacion(self):
        ...
        with open(self.general_name + '.csv', 'w') as csvfile:
            ...
            if (delta.days < 31):
                req = "/opendata/api/valores/climatologicos/" \
                    + "diarios/datos/fechaini/" \
                    + self.inidate.strftime('%Y-%m-%d') \
                    + "T00%3A00%3A00UTC" + "/fechafin/" \
                    + self.enddate.strftime('%Y-%m-%d') \
                    + "T00%3A00%3A00UTC" + "/estacion/" \
                    + self.station + "/?api_key=" + self.credentials
                ...
                req = "/opendata/api/valores/climatologicos" \
                    ...
                    + self.inidate.strftime('%Y-%m-%d') \
                    + "T00%3A00%3A00UTC" \
                    + "/fechafin/" + self.enddate.strftime('%Y-%m-%d') \
                    + "T00%3A00%3A00UTC" + "/estacion/" \
                    + self.station + "/?api_key=" + self.credentials
                print(req)
                conn.request("GET", req, headers=headers)
                ...
                for resultados in datosEstacion:
                    ...
                    try:
                        wrt = []
                        if 'velmedia' in resultados:
                            resultados['velmedia'] = resultados['velmedia'] \
                                .replace(',', '.')
                        for e in self.translate_params():
                            wrt.append(resultados[e])
                        spamwriter.writerow(wrt)
    ...
    def get_meteo(self):
        ...
        if 'Temp' in self.params:
            ...
            prefix = 'wind_'
        self.general_name = self.path + '/' \
            + self.region + '/' + prefix \
            + self.inidate.strftime('%Y-%m-%d') \
            + "_" + self.enddate.strftime('%Y-%m-%d')
        title = prefix + self.inidate.strftime('%Y-%m-%d') \
            + "_" + self.enddate.strftime('%Y-%m-%d') + ".csv"
        ...
        if (config.onedata_mode == 1):
            metadata_gen.metadata_gen(
                ...
                beginDate, endDate,
                self.region,
                str(self.lat), str(self.lon), self.params)
        return {"output": self.general_name + ".csv"}

    """Method for trasnlating from original
    parameter names to AEMET param names"""
