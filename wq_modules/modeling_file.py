        ...
        while i < len(data['date']):
            if (data['date'][i] >= ini_date) and (data['date'][i] <= end_date):
                if (len(line) == 0) and (data['date'][i] != ini_date):
                    ...
                    f.write(line)
                line = "%i %5.3f %5.3f\n" % (
                    (data['date'][i] - ini_date).seconds / 60,
...
def gen_uniform_output_bct(out_dic, output, ini_date, end_date):
    ...
    for e in out_dic:
        ...
        f.write("parameter            "
                "'time                '                     unit '[min]'\n")
        f.write("parameter            "
                "'total discharge (t)  end A'               unit '[m3/s]'\n")
        f.write("parameter            "
                "'total discharge (t)  end B'               unit '[m3/s]'\n")
...
def csv_to_bct(out_dic, output, input_csv, ini_date, end_date):
    ...
    for e in out_dic:
        data = pd.read_csv(
            input_csv + out_dic[e]['Name'] + '.csv', delimiter=';')
        ...
        if (data['date'].min() <= ini_date):
            ...
            if (data['date'][len(data['date']) - 1] < end_date):
                ...
                f.write("records-in-table    %i\n" % len(data['date']))
            while i < len(data['date']):
                if (data['date'][i] >= ini_date) and \
                   (data['date'][i] <= end_date):
                    if (len(line) == 0) and (data['date'][i] != ini_date):
    ...
    for e in out_dic:
        if "Salinity" in out_dic[e]:
            ...
            f.write("parameter            "
                    ...
                    "  unit '[min]'\n")
            f.write("parameter            "
                    "'Salinity            end A uniform'"
                    "               unit '[ppt]'\n")
            f.write("parameter            "
                    "'Salinity            end B uniform'"
                    "               unit '[ppt]'\n")
            f.write("records-in-table     2\n")
            f.write("0 %5.2f %5.2f\n" % (
                out_dic[e]['Salinity'],
        ...
        if "Temperature" in out_dic[e]:
            ...
            f.write("parameter            "
                    ...
                    "  unit '[min]'\n")
            f.write("parameter            "
                    "'Temperature           end A uniform'"
                    "               unit '[C]'\n")
            f.write("parameter            "
                    "'Temperature           end B uniform'"
                    "               unit '[C]'\n")
            f.write("records-in-table     2\n")
            f.write("0 %5.2f %5.2f\n" % (
                out_dic[e]['Temperature'],
                out_dic[e]['Temperature']))
            f.write("%i %5.2f %5.2f\n" % (
                minutes_between_date(ini_date, end_date),
                out_dic[e]['Temperature'],
...
def csv_to_dis(in_dic, output_folder, output_name, ini_date, end_date):
    ...
    for e in in_dic:
        data = pd.read_csv(
            output_folder + in_dic[e]['Name'] + '.csv', delimiter=';')
